    ...
    >
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-8 md:px-12 pb-12 sm:pb-14 md:pb-16 flex flex-col items-center">
        ...
        <div className="text-center max-w-3xl mx-auto mb-8 sm:mb-10 space-y-3">
          ...
          {/* Device Mockup Toggle Switch */}
          <div className="pt-2 flex items-center justify-center gap-2">
            <div className="inline-flex items-center p-1 rounded-lg bg-slate-100/90 border border-slate-300/80 text-xs shadow-inner">
              <button
                type="button"
              ...
              >
                ...
                <span>{language === "hi" ? "व्हाइट डेस्कटॉप" : "White Desktop"}</span>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <span>{language === "hi" ? "व्हाइट लैपटॉप" : "White Laptop"}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-4xl mx-auto relative">
          ...
          {/* Outer WHITE Monitor Frame Chassis */}
          <div
            className={`transition-all duration-300 relative ${
              deviceType === "desktop"
                ? "bg-white border-[5px] sm:border-[6px] border-slate-200/90 rounded-t-2xl sm:rounded-t-3xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.14)] p-2 sm:p-3 pb-0"
                : "bg-white border-[5px] sm:border-[6px] border-slate-200/90 rounded-t-2xl sm:rounded-t-3xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.14)] p-2 sm:p-2.5 pb-0"
            }`}
          >
            {/* Top Bezel Webcam Notch / Sensor Dot (White Desktop) */}
            <div className="flex items-center justify-center pb-2">
              <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-slate-100 border border-slate-200/80 shadow-inner">
                <div className="w-1.5 h-1.5 rounded-full bg-slate-300 ring-1 ring-slate-400/50 flex items-center justify-center">
                  <div className="w-0.5 h-0.5 rounded-full bg-emerald-500 animate-pulse" />
                </div>
                <span className="text-[8px] font-mono text-slate-500 uppercase tracking-widest hidden sm:inline">
                  FHD Sensor
                </span>
              </div>
            </div>
            ...
            <div
              ...
              className={`w-full aspect-video bg-black relative rounded-lg sm:rounded-xl overflow-hidden shadow-inner group ${
                isFullscreen ? "fixed inset-0 z-50 rounded-none aspect-auto w-screen h-screen" : ""
              }`}
            >
              {isPlaying ? (
            ...
            {/* Bottom Monitor Chin Bar (White Desktop) */}
            <div className="h-7 sm:h-8 bg-slate-50 border-t border-slate-200/90 flex items-center justify-between px-3 text-[10px] text-slate-600 font-mono">
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-[9px] sm:text-[10px] text-slate-600 font-semibold">
                  STATUS: {isPlaying ? "STREAMING (4K)" : "ONLINE"}
                </span>
              </div>
              <span className="text-[9px] sm:text-[10px] font-bold tracking-wider text-[#133E87]">
                NIDHI-RAKSHAK WHITE WORKSTATION
              </span>
              <span className="text-[9px] sm:text-[10px] text-slate-500 hidden sm:inline font-semibold">
                SIH2026/CODE_WARRIORS
              </span>
            </div>
          </div>

          {/* Device Base: White Metallic Desktop Stand OR White Laptop Deck */}
          {deviceType === "desktop" ? (
            /* White Desktop Metallic Stand */
            <div className="w-full flex flex-col items-center">
              {/* Stand Neck */}
              <div className="w-20 sm:w-28 h-6 sm:h-8 bg-gradient-to-b from-slate-100 via-slate-200 to-slate-300 shadow-xs border-x border-slate-300/80" />

              {/* Stand Base Plate */}
              <div className="w-44 sm:w-64 h-3 sm:h-4 bg-gradient-to-r from-slate-200 via-white to-slate-200 rounded-full shadow-md border-t border-white border-b border-slate-300/60" />

              {/* Ambient Desk Contact Shadow */}
              <div className="w-60 sm:w-80 h-3 bg-slate-400/25 blur-md rounded-full -mt-1 pointer-events-none" />
            </div>
          ) : (
            /* White Laptop Bottom Deck & Keyboard Base */
            <div className="w-full flex flex-col items-center">
              {/* Laptop Unibody White/Silver Base Deck */}
              <div className="w-[104%] -mx-[2%] h-4 sm:h-5 bg-gradient-to-b from-white via-slate-100 to-slate-200 rounded-b-xl border-t border-white border-b border-slate-300/60 shadow-lg flex items-center justify-center relative">
                {/* Thumb Groove / Notch for opening lid */}
                <div className="w-16 sm:w-20 h-1.5 bg-slate-300 rounded-b-md shadow-inner" />
              </div>

              {/* Ambient Contact Shadow */}
              <div className="w-[108%] h-4 bg-slate-400/25 blur-md rounded-full -mt-1 pointer-events-none" />
            </div>
          )}
        </div>
