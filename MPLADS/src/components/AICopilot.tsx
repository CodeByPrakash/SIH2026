...
export default function AICopilot({ onNavigate, user }: AICopilotProps) {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      const userPayload = {
        name: userName,
        role: userRole,
        constituency: accountUser?.constituency,
        district: accountUser?.district,
  ...
  }, [loading, userName, userRole, accountUser, messages]);
  ...
  return (
    <>
        ...
        <div className="fixed z-50 flex flex-col shadow-2xl animate-slide-in" style={{ bottom: 88, right: 24, width: 380, height: 560, borderRadius: 20, background: "#FFFFFF", border: "1px solid #E2E8F0", overflow: "hidden" }}>
          ...
          <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0" style={{ background: "linear-gradient(135deg, #0D1B3E 0%, #1a3a6b 100%)" }}>
            ...
            <div className="flex-1 min-w-0">
              <div className="text-white font-display font-bold text-sm leading-tight flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className="text-blue-100 text-[10.5px] font-medium truncate max-w-[130px]" title={`Account: ${userName}`}>
                  {userName}
                </span>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-4 bg-slate-50/60">
            {messages.length <= 1 && (
              <div className="text-center py-3">
                <div className="w-10 h-10 rounded-2xl mx-auto mb-2 flex items-center justify-center shadow-md" style={{ background: "linear-gradient(135deg, #0D1B3E, #1a3a6b)" }}>
                  <svg viewBox="0 0 24 24" fill="#F59E0B" className="w-5 h-5"><path d="M12 2a2 2 0 0 1 2 2c0 .74-.4 1.39-1 1.73V7h1a7 7 0 0 1 7 7h1a1 1 0 0 1 0 2h-1v1a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-1H2a1 1 0 0 1 0-2h1a7 7 0 0 1 7-7h1V5.73c-.6-.34-1-.99-1-1.73a2 2 0 0 1 2-2M7.5 13A2.5 2.5 0 0 0 5 15.5 2.5 2.5 0 0 0 7.5 18 2.5 2.5 0 0 0 10 15.5 2.5 2.5 0 0 0 7.5 13m9 0A2.5 2.5 0 0 0 14 15.5a2.5 2.5 0 0 0 2.5 2.5 2.5 2.5 0 0 0 2.5-2.5A2.5 2.5 0 0 0 16.5 13z"/></svg>
                </div>
      ...
      {/* FAB */}
      <button onClick={handleToggleOpen} className="fixed z-50 flex items-center justify-center rounded-full shadow-2xl transition-all duration-200 active:scale-95 hover:scale-105" style={{ bottom: 24, right: 24, width: 56, height: 56, background: open ? "linear-gradient(135deg, #EF4444, #DC2626)" : "linear-gradient(135deg, #0D1B3E 0%, #1a3a6b 60%, #F59E0B 200%)", boxShadow: open ? "0 8px 32px rgba(239,68,68,0.45)" : "0 8px 32px rgba(13,27,62,0.55)" }} title="NIDHI-RAKSHAK AI Copilot" aria-label="Open AI Copilot">
        {open
